...
@Component({
  selector: 'app-admin-humidity',
  standalone: true,
  imports: [CommonModule, RouterModule],
  template: `
    <div class="admin-humidity-container">
      <!-- Header -->
      <header class="admin-humidity-header">
        <div class="header-content">
          <div class="header-left">
            <button (click)="goBack()" class="back-btn">← Back</button>
            <h1>💧 Admin Humidity Monitoring</h1>
          </div>
          <div class="user-info">
      ...
      <!-- Main Content -->
      <main class="admin-humidity-main">
        <!-- Current Reading -->
  ...
  `,
  styles: [`
    .admin-humidity-container {
      min-height: 100vh;
      background: #f8f9fa;
    }

    .admin-humidity-header {
...
export class AdminHumidityComponent implements OnInit {
  currentUser: any;
  ...
  ngOnInit() {
    ...
    // Refresh current humidity and logs every 20 seconds
    setInterval(() => {
      this.loadCurrentHumidity();
      this.loadHumidityLog(this.selectedLimit);
    }, 20000);
  ...
  downloadData(limit: number = 100) {
    this.dataService.downloadHumidityLog(limit).subscribe({
      next: (blob) => {
        ...
        a.href = url;
        a.download = `admin_humidity_log_${new Date().toISOString().split('T')[0]}.csv`;
